...
export default function SpinNumber({ min, max }: { min: number; max: number }) {
      ...
      slotProps={{
        root: (ownerState: NumberInputOwnerState) => {
          ...
          return {
            ...resolvedSlotProps,
            className: clsx(
              "grid grid-cols-[1fr_8px] grid-rows-2 overflow-hidden font-sans rounded-[4px] text-slate-900 dark:text-slate-300 border border-solid  bg-white dark:bg-slate-900  hover:border-violet-400 dark:hover:border-violet-400 focus-visible:outline-0 p-1",
              ownerState.focused
